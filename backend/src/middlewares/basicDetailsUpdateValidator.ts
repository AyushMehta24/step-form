  ...
  technologyKnown: Joi.array()
    .items(
      Joi.object({
        ...
        technologyRead: Joi.boolean(),
        technologyWrite: Joi.boolean(),
        technologySpeak: Joi.boolean(),
      })
        .or("technologyRead", "technologyWrite", "technologySpeak")
        .required()
    )
    .required()
    .messages({
...
const validateBasicDetailsUpdate = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  if (req.body.educationDetails && Array.isArray(req.body.educationDetails)) {
    ...
    if (req.body.educationDetails.length === 0) {
      return res
        .status(400)
        .json({ errors: ["Education Details cannot be empty"] });
    }
  }
  ...
  if (req.body.preferences && Array.isArray(req.body.preferences)) {
    ...
    if (req.body.preferences.length === 0) {
      return res.status(400).json({ errors: ["Preferences cannot be empty"] });
    }
  }
...
export default validateBasicDetailsUpdate;
